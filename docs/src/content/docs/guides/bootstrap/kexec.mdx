...
The `<arch>` placeholder is automatically replaced with the detected architecture (e.g. `x86_64`). Panix detects whether `image` is a local path or an HTTP URL: local paths are transferred via `rsync`, URLs trigger a remote download with `curl`.
