...
Revert machines to their pre-deploy generation automatically when activation fails.
...
3. It re-runs the activation against that closure, using the preset's default activation mode — and the `nix-env --profile --set` step before it, for types whose preset sets profiles (`nixosConfigurations`, `darwinConfigurations`, `systemConfigs`); profile-tracking types without profile-setting (`homeConfigurations`, `nixOnDroidConfigurations`) roll back the activation only.
...
- The workflow was cancelled (a cancelled context would make the rollback commands fail instantly, burying the real activation error under rollback noise).
...
To switch to an earlier generation explicitly, use the standalone `panix rollback` command, see [Other Commands](/cli/other-commands/).

See the [phase pipeline](/concepts/phases/) for how rollback fits into the deploy flow.
