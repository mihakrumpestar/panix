...
flags: # Listed are default values, all also overridable using CLI arguments
  activation_mode:                     # Activation mode override (YAML requires object form; CLI accepts bare value)
    alltypes: test                     # Applies to all installable types
    # pertype:                         # Or per type:
  ...
  timeout: 2h                          # Timeout per command (e.g. '1h', '1m15s')
  ...
  out_links: false                     # Create GC root outlinks for built installables (see Nix Command Flags)
  out_links_dir: .panix                # Directory for build outlinks
  ...
  validate:                            # Validation settings (both forced on for 'panix eval')
    flakes: false                      # Validate flake URLs and configuration keys
    bootstrap_secrets: false           # Validate that bootstrap disk encryption key local paths exist
  ...
  tui:
    ...
    command_output_max_lines: 10000    # Max output lines kept per command (older trimmed from the front)
  logging:
  ...
  runtime:                             # Operational timeouts and concurrency
    max_concurrency: 0                 # Max parallel workers in the worker pool (0 = unlimited)
...
fleet:
  ...
  auto_rollback: false                 # Roll back to pre-deploy generation on activation failure (see auto-rollback feature page)
  nix:
